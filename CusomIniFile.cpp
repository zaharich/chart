...
IniSettings::IniSettings(AnsiString& nameIniMain)
{
 // mainIni
 iniFileNameMain = nameIniMain;
 if(! FileExists(iniFileNameMain))
 {
     showErrMessage(MAININI_ERR);
     return;
 }
 iniFileMain = new TIniFile(iniFileNameMain);

 // anyIni
 iniFileNameAny = iniFileMain->ReadString("Files", "LastFileIni", "");
