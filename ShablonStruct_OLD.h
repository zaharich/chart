...
struct KTF                    //
      {
       char NameNak[16];      // Наименование накопителя
       char TipNak[16];       // Наименование накопителя
  short int NKTF;             // №  КТФ
  short int KBxodKTF;         // К-во входов  КТФ
  short int NBxod[32];        // № входов накопителя
  short int NBxodKTF[32];     // № входов накопителя для КТФ
      };
*/
/*
struct tf48_Index
{
int   IndexTipFile;              // Индекс начала символьного массива char[256], определяющего тип файла например(tf48)
int   IndexStruct_tf48_Index;    // Индекс начала структуры индексов
int   IndexStruct_tf48_Shapka;    // Индекс начала структуры шапки
int   IndexStruct_tf48_Pasp;        // Индекс начала структур паспортов
int   IndexStruct_tf48_RK;          // Индекс начала единого массива структур RK
int   IndexStruct_tf48_PozNPasp;    // Индекс индексного блока
int   IndexStructDate;        // Индекс начала данных
int   IndexStructGif;          // Индекс начала GIF
int   LGif;                          // Длина  Gif
//char rezerv[232];
};
...
char    Nakopital[16];
char    DataPolet[16];
short  int    F;              // Частота Мах
short  int    KPasp;          // К-во паспортов
short  int    KRK;            // К-во разовых команд
int KStructIndex;             // Кол-во структур в индексном блоке для tf48
float    Tmin;                // Время начала полета
float    Tmax;                // Время конца  полета
//               char rezerv[194];
};

struct  tf48_Pasp                              // Структура   паспортов параметров
                 {
                 char   Razdel[16];
                 char   Ident[16];
                 char    Name[128];    //  Наименование параметра
                 char    Razm[16];    // Размерность
                 char   TipS[16];        //  Тип параметра : F,K,k,R,T
                   int    Index;            //  Индекс массива значений параметра
                  int     LBlok;           //   Длина  массива
...
struct  tf48_RK                           // Структура   RK
                 {
                 char   NameRK[128];    // Наименование RK
                 short   int    NRazRK;                   //  № разряда
//                 short   int    KRK;                //  К-во RK в слове
                 short int NRK_SL;        // № следующей структуры RK c одинаковым идентификатором
                };

struct  tf48_PozNPasp
{
         unsigned int NSis :4;       // Номер системы для Arinc
         unsigned int NCL :4;        // Status=0 - Позиция пересылается в паспорт по номеру Ncl
         unsigned int KPasp :8;      // Количество паспортов на одной позиции кадра
         unsigned int NPasp :16;     // № паспортa  tf48_Pasp
};
*/
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
